...
def upgrade() -> None:
    ...
    op.add_column('flights', sa.Column('price', sa.DECIMAL(precision=10, scale=2), nullable=True))
    op.add_column('flights', sa.Column('status', sa.Enum('SCHEDULED', 'DELAYED', 'CANCELED', name='statustype', create_type=False), nullable=True))
    # Важно: create_type=False, потому что тип мы уже создали вручную

    # Заполняем дефолтными значениями существующие записи
    op.execute("UPDATE flights SET arrival_time = NOW(), price = 0.00, status = 'SCHEDULED'")
...
def downgrade() -> None:
    ...
    op.create_index(op.f('ix_accesstoken_created_at'), 'access_token', ['created_at'], unique=False)