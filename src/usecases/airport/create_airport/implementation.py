...
from usecases.airport.create_airport.abstract import AbstractCreateAirportUseCase  # ← путь №3, вообще другой
...
class PostgreSQLCreateAirportUseCase(AbstractCreateAirportUseCase):
    ...
    async def execute(self, schema: CreateAirportSchema):
        data = AirportCreateData(name=schema.name, code=schema.code,
                                 city=schema.city, country=schema.country)
